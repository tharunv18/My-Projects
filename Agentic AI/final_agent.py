...
if __name__ == "__main__":
    ...
    for idx, scenario in enumerate(PATIENT_SCENARIOS, start=55):
        PATIENT_PROMPT = scenario
        print(f"\n########## SCENARIO {idx:03d} ##########\n")
        for lbl, mid in THERAPIST_ENGINES.items():
            run_ensemble_session(lbl, mid, turns=12)
        for lbl, mid in BASELINE_MODELS.items():
            try:
                run_session_with_ratings_baseline(f"Baseline – {lbl}", mid, turns=12)
            except Exception:
